  ...
  <div>
    ...
    <el-table :data="list.slice((currentPage-1)*PageSize,currentPage*PageSize)" border stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="175">
        <template slot-scope="scope">
          <el-button type="primary" :disabled="scope.row.status >2" size="small" @click="updateInfo(scope.row.oid,'3')"
            >收货</el-button
          >
          <el-button type="primary" :disabled="scope.row.status >= 4 " style="background:red" size="small" @click="updateInfo(scope.row.oid,'4')"
            >退货</el-button
          >
          <!--删除-->
        </template>
...
addClass(i) {
    ...
    async getList() {
      ...
      const { data: res } = await this.$http.post("/order/getMyOrder",this.queryForm);

      this.list = res.myOrder;
